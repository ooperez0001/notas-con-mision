...
export const SmartBible: React.FC<SmartBibleProps> = ({
  ...
  const tt = (key: string) => (t as any)[key] ?? key;
  ...
  // const getPassageKey = (p: any) => `${p.id}|${p.reference}|${p.version}`;
  useEffect(() => {
    if (selectedPassages.length === 0) {
      setActivePassageKey(null);
      return;
    }

    // Si no hay activo o ya no existe en la lista, activa el último
    const exists = activePassageKey
      ? selectedPassages.some((p) => getPassageKey(p) === activePassageKey)
      : false;

    if (!activePassageKey || !exists) {
      const last = selectedPassages[selectedPassages.length - 1];
      setActivePassageKey(getPassageKey(last));
    }
  }, [selectedPassages]); // intencional: no meto activePassageKey para evitar loops

  // AI State
  const [aiContent, setAiContent] = useState<{ [key: string]: string }>({});
  const [openAIAccordions, setOpenAIAccordions] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const handleAddFromInput = async () => {
    ...
    if (isVerse) {
      try {
        ...
        const text = baseText ? `${verseNumber}. ${baseText}` : "";
          ...
          >
            ...
            <div className="px-3 py-2 border-b last:border-b-0">
              {chapterSuggestions.map((s) => (
                <div
                  key={`${s.reference}-${(s as any).version || ""}`}
                  className="mb-3"
                >
                  {/* Encabezado */}
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2 mb-1">
                ...
                <div className="flex gap-2 items-center">
                  {/* ✅ Activo / Hacer activo */}
                  {getPassageKey(p) === activePassageKey ? (
                    <span className="text-[11px] px-2 py-1 rounded-full bg-green-200 text-green-900">
                      {tt("active")}
                    </span>
                  ) : (
                    <button
                      type="button"
                      onClick={() => setActivePassageKey(getPassageKey(p))}
                      className="text-[11px] px-2 py-1 rounded-full border border-green-300 bg-white hover:bg-green-100"
                      title={tt("useForAi")}
                    >
                      {tt("makeActive")}
                    </button>
      ...
      {false && verseData && !loading && (
        <div className="space-y-8 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-2xl font-bold text-blue-700 dark:text-blue-400 mb-6 border-b border-gray-100 dark:border-gray-700 pb-2">
              {verseData.ref}
            </h2>
              ...
              .map(([version, verses]) => {
                ...
                return (
                  <div key={version} className="mb-6 last:mb-0">
                    <p className="font-bold text-xs text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700 inline-block px-2 py-1 rounded mb-2">
                      {version}
                    </p>
                    ...
                    <div className="text-gray-800 dark:text-gray-200 leading-relaxed space-y-2">
                      {versesList.map((v) => (
                        <span
                          ...
                          className={
                            v.isJesusWords
                              ? "text-red-700 dark:text-red-400 block"
                              : "block"
                          }
                        >
                          <strong className="pr-2 text-blue-300 dark:text-blue-500 text-sm font-bold align-top">
                            {v.number}
                          </strong>
                          {v.text}
                        </span>
      ...
      {keywordResults.length > 0 && !loading && (
        <div className="space-y-4 animate-fade-in">
          ...
          {keywordResults.map((verse, index) => (
            <div
              key={index}
              className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:border-blue-200 dark:hover:border-blue-700 transition-colors"
            >
