...
async function fetchDict(lang: string, q: string) {
  const res = await fetch(
    `https://api.dictionaryapi.dev/api/v2/entries/${lang}/${encodeURIComponent(q)}`
  );
...
async function fetchWiktionaryDefinition(lang: "es" | "en" | "pt", term: string) {
  const res = await fetch(
...
if (language !== "en") {
  try {
    ...
    setDictError("No se pudo generar la definición. Revisa tu conexión o tu API key.");
  } finally {
...
for (const e of entries) {
  ...
  for (const defObj of defList) {
    ...
    const isJunk =
      ...
      /^Alternative (form|spelling) of/i.test(text.replace(/^.*?:\s*/, ""));
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`shrink-0 px-4 py-2 rounded-xl text-sm font-semibold text-white ${addingPassage || !newPassageRef.trim()
          ...
          {versionSuggestions.length > 0 && (
            <div
              className={`mt-2 rounded-xl bg-white shadow-sm max-h-[320px] overflow-y-auto border ${keepOpen ? "border-blue-500 ring-1 ring-blue-200" : "border-gray-200"
                }`}
            >
              {keepOpen && (
  ...
  <div className="mt-3 max-h-56 overflow-auto rounded-lg border border-gray-200 p-3">
    {dictResults.lang ? (
      <div className="mb-2 text-xs text-gray-500">
       Idioma: {dictResults.lang === "es" ? "Español" : dictResults.lang === "pt" ? "Portugués" : dictResults.lang === "en" ? "Inglés" : dictResults.lang}
    ...
    <ul className="list-disc pl-5 space-y-1">
      ...
      {/* Guardadas */}
      <div className="mt-4">
        <div className="text-sm font-semibold">Palabras guardadas</div>
              ...
              <div key={w.term} className="mb-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setSavedWords((prev) => prev.filter((x) => x.term !== w.term))
                    }
