...
export const SermonsList: React.FC<SermonsListProps> = ({
  sermons,
  setSermons,
  user,
  onOpenPremium,
  ...
  const [internalSelectedSermon, setInternalSelectedSermon] =
  ...
  const handleDeleteSermon = (id: string) => {
    ...
    setSermons((prevSermons) => {
      ...
      try {
        ...
        console.error(
          "Error al actualizar los sermones en localStorage",
          error,
        );
      }
  ...
  const filteredSermons = sermons.filter((sermon) => {
    ...
    return (
      normalizeText(sermon.title).includes(query) ||
      ...
      <div className="space-y-4">
        {filteredSermons.length > 0 ? (
          filteredSermons.map((sermon) => {
            ...
            const refKey = (p: any) =>
              normKey(
                p?.reference ?? p?.ref ?? (typeof p === "string" ? p : ""),
              );
            ...
            keyOrder.forEach((p: any, i: number) =>
              orderIndex.set(refKey(p), i),
            );
            ...
            return (
              ...
              >
                ...
                {searchQuery &&
                  normalizeText(sermon.notes).includes(
